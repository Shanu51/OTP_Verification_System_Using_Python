...
receiver_email = input("enter ur email id:").lower()
...
def email_verification(receiver_email):
    ...
    if "@" not in receiver_email or count != 2:
        ...
        new_receiver_email = input("enter correct email id:").lower()
...
server.sendmail("your_email", valid_receiver_email, message)
...
def sending_otp(receiver_email):
    ...
    if received_OTP == new_otp:
        ...
        else_resend_otp()
def else_resend_otp():
    answer = input("Enter 'yes' to resend OTP to the same email or 'no' to enter a new email id: ").lower()
    YES = ['yes']
    NO = ['no']
    if answer in YES:
        sending_otp(valid_receiver_email)
    elif answer in NO:
        new_receiver_email = input("Enter new email id: ").lower()
        new_valid_receiver_email = email_verification(new_receiver_email)
        sending_otp(new_valid_receiver_email)
    else:
        print("Invalid input")
